...
class AuthService:
    ...
    @staticmethod
    async def create_ldap_user(db, username, nickname, email):
        """
        创建LDAP用户的共用方法
        """
    ...
    async def ldap_user_verify(username: str, password: str) -> User:
        ...
        try:
            ...
            if not user:
                # 准备用户数据
                nickname = ldap_result[3]
                email = ldap_result[1]
                try:
                    db = async_db_session()
                    await db.begin()
                    user = await AuthService.create_ldap_user(db, username, nickname, email)
                    await db.commit()
    ...
    async def swagger_ldap_login(self, *, obj: HTTPBasicCredentials) -> tuple[str, User]:
        ...
        async with async_db_session.begin() as db:
            ...
            if not user:
                ...
                email = ldap_result[1]
                try:
                    user = await AuthService.create_ldap_user(db, obj.username, nickname, email)
                    log.info(f"Successfully created user: {obj.username}")
                except Exception as e:
